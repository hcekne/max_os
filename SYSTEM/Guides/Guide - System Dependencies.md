...
sh "$(git rev-parse --show-toplevel)/AUTOMATE/Skills/tools/ensure_local_setup.sh"
python3 "$(git rev-parse --show-toplevel)/AUTOMATE/Skills/tools/check_dependencies.py"
```
...
| `python3` ≥ 3.10 | `brew install python@3` | Every Python tool in `AUTOMATE/Skills/tools/`. |
...
| Dependency | Notes |
| --- | --- |
| Pre-commit hooks | Run `sh AUTOMATE/Skills/tools/ensure_local_setup.sh` once after cloning. Writes `.maxos/local_setup_status.yaml` and registers `.githooks` as the hooks path so the knowledge-lint and whitespace gate run before each commit. |
...
| --- | --- | --- | --- |
| **Slide deck generation** (`Skill - Slide Deck Generation`, `AUTOMATE/Skills/tools/slides/build_deck.py`) | `pandoc` | `brew install pandoc` | Falls back to the inline Markdown subset parser. Most slides still render, but features like advanced tables and pipe-table-with-formatting can degrade. Visually inspect every slide. |
...
| **Slide deck visual QA** (`AUTOMATE/Skills/tools/slides/check_deck_visual.mjs`) | `node` ≥ 18, `playwright` ≥ 1.40 with the Chromium browser | `brew install node` then `cd .maxos/visual-check && npm install playwright && npx playwright install chromium` | The visual QA cannot run. Without it, the agent has no way to verify the rendered slides before reporting completion. **This is a hard requirement for the slide-deck skill**, not an optional extra. |
| **Project-specific HTML/PDF pack builds** (`KNOWLEDGE/Projects/<project>/.../build_pack.py`) | `pandoc`, `weasyprint` | `brew install pandoc weasyprint` | Pack HTML and PDF cannot be regenerated. Previously generated PDFs may remain on disk but stale. |
...
| **Word document export** (`Skill - Export Markdown to Word Document`, `AUTOMATE/Skills/tools/md_to_docx.py`) | Python standard library only — writes the DOCX OOXML directly. No `python-docx` install needed. | — | None. Self-contained. |
| **Vault validator** (`AUTOMATE/Skills/tools/check_vault.py`) | Python standard library only | — | None. |
| **Knowledge lint and quality gate** (`AUTOMATE/Skills/tools/maxos_quality_gate.py`, `knowledge_lint.py`) | Python standard library only | — | None. Hook fails commit if quality gate detects issues. |
...
After building a deck, **always** run `node AUTOMATE/Skills/tools/slides/check_deck_visual.mjs <deck.html>` and **read every screenshot as image bytes**, not just the report JSON. See `Skill - Slide Deck Generation.md` for the mandatory visual QA flow.
...
Self-contained — Python stdlib only. Run via `python3 AUTOMATE/Skills/tools/md_to_docx.py <source.md> [--output <dest.docx>]` per the skill card.

---

## Verification script

`AUTOMATE/Skills/tools/check_dependencies.py` prints a one-shot report on which dependencies are available and which are missing, with the install command for each gap.
...
3. Add the dependency check to `AUTOMATE/Skills/tools/check_dependencies.py`.
