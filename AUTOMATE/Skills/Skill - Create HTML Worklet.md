...
- Optional output filename under `AUTOMATE/Modules/Worklets/`

## Steps
1. Read [[SYSTEM/Document Model]], [[SYSTEM/Rendering Policy]], [[SYSTEM/Artifact Safety Policy]], and [[SYSTEM/Worklet Conventions]].
2. Confirm the worklet belongs in `AUTOMATE/Modules/Worklets/` and remains optional.
3. Define the input contract, output contract, and whether JSON state is needed.
4. Start from `SYSTEM/Templates/TPL - HTML Worklet.html` unless a more specific existing worklet is provided.
...
7. Create a worklet manifest from `SYSTEM/Templates/TPL - Worklet Manifest.md`.
...
## Outputs
- HTML worklet in `AUTOMATE/Modules/Worklets/`
