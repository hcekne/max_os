...
```bash
python3 AUTOMATE/Skills/tools/knowledge_lint.py --root . --fail-on error
```

For a Markdown report:

```bash
python3 AUTOMATE/Skills/tools/knowledge_lint.py --root . --changed-only --format markdown --output "SYSTEM/Proposals/Knowledge Lint Report - YYYY-MM-DD.md" --fail-on none
```
