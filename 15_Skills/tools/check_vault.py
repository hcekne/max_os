...
Usage:
    python 15_Skills/tools/check_vault.py            # auto-detects vault root
    python 15_Skills/tools/check_vault.py --root .   # explicit root
...
FRONTMATTER_REQUIRED = {
    ...
    "13_Goals":         "Goal - *.md",
    "15_Skills":        "Skill - *.md",
    "12_Workflows":     "Workflow - *.md",
    "03_Clients":       "Client - *.md",
    "04_Projects":      "Project - *.md",
    "99_Templates":     "TPL - *.md",
}
...
NAMING_RULES = [
    ("04_Projects",    "Project - *.md",    re.compile(r"^Project - .+\.md$")),
    ("13_Goals",       "Goal - *.md",       re.compile(r"^Goal - .+\.md$")),
    ("12_Workflows",   "Workflow - *.md",   re.compile(r"^Workflow - .+\.md$")),
    ("15_Skills",      "Skill - *.md",      re.compile(r"^Skill - .+\.md$")),
    ("99_Templates",   "TPL - *.md",        re.compile(r"^TPL - .+\.md$")),
    ("07_Daily",       "YYYY-MM-DD.md",     re.compile(r"^\d{4}-\d{2}-\d{2}\.md$")),
    ("06_Interactions","YYYY-MM-DD ... .md", re.compile(r"^\d{4}-\d{2}-\d{2}([ \-.].*)?\.md$")),
]
...
SKIP_LINK_CHECK_PREFIXES = ("16_Cleaning/", "99_Templates/")
...
def check_hygiene(root: Path, findings: list) -> None:
    ...
    # Numbered-folder mirror files at root (e.g. 06_Interactions.md).
    for p in root.glob("[0-9][0-9]_*.md"):
        findings.append(("hygiene", p.name, "numbered folder-mirror file at root"))


def check_system_state(root: Path, index: dict, folders: set, findings: list) -> None:
    p = root / "00_System" / "System State.md"
    if not p.exists():
        findings.append(("system-state", "00_System/System State.md", "missing"))
        return
...
def main() -> int:
    ...
    ap.add_argument("--root", default=None,
                    help="Vault root (default: ascend from CWD until a 00_System/ is found)")
    args = ap.parse_args()

    if args.root:
        root = Path(args.root).resolve()
        if not (root / "00_System").is_dir():
            print(f"error: {root} does not contain 00_System/", file=sys.stderr)
            return 0
    else:
        found = locate_root(Path.cwd().resolve())
        if found is None:
            print("error: could not locate vault root (no 00_System/ found from CWD upward)",
                  file=sys.stderr)
            return 0
        root = found
